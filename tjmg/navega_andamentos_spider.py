...
class TjMGSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        cod_comarca = "702"
        csv_limpo = Path(f"resultados/proc_uberlandia_47.csv")
        df_csv_limpo = pd.read_csv(csv_limpo, index_col=0, dtype="str", names=["num_proc"])
    ...
    def parse_new(self, response):
        ...
        if numero != 0:
            if check_disp != 3:
                ...
                vara = response.xpath('//table/tr/td/b/text()').re_first(r"((INFÂNCIA|\d{1,2}ª|CR.PESSOA/PREC.CRIME|EXECUÇÕES|JESP CÍVEL) "
